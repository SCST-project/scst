 ...
 * @ct_pkt: CT request and response buffer
 * @cmd: GS command
 * @rsp_size: response size in bytes
 *
...
qla2x00_prep_ct_req(struct ct_sns_pkt *ct_pkt, uint16_t cmd, uint16_t rsp_size)
{
	struct ct_sns_req *ct_req = &ct_pkt->p.req;

	memset(ct_pkt, 0, sizeof(struct ct_sns_pkt));
...
qla2x00_rft_id(scsi_qla_host_t *vha)
{
	...
	/* Prepare CT request */
	ct_req = qla2x00_prep_ct_req(ha->ct_sns, RFT_ID_CMD, RFT_ID_RSP_SIZE);
	ct_rsp = &ha->ct_sns->p.rsp;

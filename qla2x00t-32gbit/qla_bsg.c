...
static int qla28xx_validate_flash_image(BSG_JOB_TYPE *bsg_job);
...
static int
qla2x00_get_drv_attr(BSG_JOB_TYPE *bsg_job)
{
...
static bool qla_bsg_found(struct qla_qpair *qpair, BSG_JOB_TYPE *bsg_job)
{
...
static int
qla28xx_do_validate_flash_image(BSG_JOB_TYPE *bsg_job, uint16_t *state)
{
	struct fc_bsg_request *bsg_request = bsg_job->request;
	scsi_qla_host_t *vha = shost_priv(fc_bsg_to_shost(bsg_job));
	uint16_t mstate[16];
